...
def modular_addition_task_performance(inputs: CreateMetricsInputs) -> Mapping[str, float | int | wandb.Table]:
    """Evaluate performance on the modular addition task."""
    # Import here to avoid circular imports  
    from spd.experiments.modular_addition.plotting import create_modular_addition_metrics
    
    return create_modular_addition_metrics(
        model=inputs.model,
        components=inputs.components,
        gates=inputs.gates,
        causal_importances=inputs.causal_importances,
...
METRICS_FNS = [
    ...
    lm_embed,
    lm_ce_losses,
    modular_addition_task_performance,
]
...
def modular_addition_ci_plots(inputs: CreateFiguresInputs) -> Mapping[str, plt.Figure]:
    """Create modular addition-specific CI plots showing component activation patterns."""
    # Import here to avoid circular imports
    from spd.experiments.modular_addition.plotting import create_modular_addition_plot_results
    
    return create_modular_addition_plot_results(
        model=inputs.model,
        components=inputs.components,
        gates=inputs.gates,
        device=inputs.device,
        p=113,  # Standard modular addition prime
    )
